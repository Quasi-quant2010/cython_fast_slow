...
def matmult3_1(np.ndarray[DOUBLE_t, ndim=2] a, np.ndarray[DOUBLE_t, ndim=2] b):
    cdef int n, m, l    
    cdef double *c
    ...
    matmult_intrn(n, m, l, <double*>a.data, <double*>b.data, c)
    
    return [ c[i] for i in xrange(matrix_size)]

@cython.boundscheck(false)
def matmult3_2(np.ndarray[DOUBLE_t, ndim=2] a, np.ndarray[DOUBLE_t, ndim=2] b):
    cdef int n, m, l    
    cdef double *c
    cdef int matrix_size
